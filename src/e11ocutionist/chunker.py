...
def generate_id(prev_id: str, content: str) -> str:
    ...
    if not prev_id:
        ...
        prev_suffix = prev_id.rsplit("-", maxsplit=1)[-1]
...
def update_tok_attributes_on_tree(root: etree._Element) -> None:
    ...
    for element in root.xpath("//*[@tok='____' or not(@tok)]"):
        try:
            ...
            if (
                not content_parts and len(element) > 0
            ):  # Element with children but no direct text
                element_xml_for_tokens = etree.tostring(
                    element, encoding="utf-8", method="xml"
                ).decode("utf-8")
            else:
                element_xml_for_tokens = (
...
def add_unit_tags_on_tree(
    root_element: etree._Element, semantic_boundaries: list[tuple[str, str]]
) -> etree._Element:
    ...
    new_children_for_root: list[etree._Element] = []
    current_unit_items: list[etree._Element] = []
...
def create_chunks_on_tree(
    root_doc_element: etree._Element, max_chunk_size: int
) -> etree._Element:
    ...
    if not units:
        ...
        if list(root_doc_element) and not root_doc_element.xpath(
            "./chunk"
        ):  # If <doc> has items but no chunks
    ...
    for unit_node in units:
        unit_xml_for_tokens = etree.tostring(unit_node, encoding="utf-8").decode(
            "utf-8"
        )
        ...
        if (
            is_chapter_start and current_chunk_tokens > 0
        ):  # Chapters always start new chunks if current isn't empty
            new_chunk_forced = True
        elif (
            unit_tokens > max_chunk_size and current_chunk_tokens > 0
        ):  # Oversized unit needs new chunk if current isn't empty
            new_chunk_forced = True
        elif (
            current_chunk_tokens + unit_tokens > max_chunk_size
    ...
    logger.info(
        f"Organized units into {len(root_doc_element.xpath('./chunk'))} chunks."
    )
    return root_doc_element
...
def create_backup(file_path: str | Path, stage: str = "") -> Path | None:
    ...
    backup_name = (
        ...
        if stage
        else f"{file_path.stem}_{timestamp}{file_path.suffix}"
    )
...
def save_tree_to_file(
    root: etree._Element, file_path: str | Path, pretty: bool = False
) -> None:
    xml_string = etree.tostring(
        root, encoding="utf-8", method="xml", xml_declaration=True, pretty_print=pretty
    ).decode("utf-8")
...
def process_document(
    ...
    try:
        ...
        items_xml_str_list = [
            item_tuple[0] for item_tuple in create_item_elements(itemized_paragraphs)
        ]
        ...
        if backup:
            ...
            )  # Ensure output dir exists for backup
        ...
        current_doc_str_for_llm = etree.tostring(current_root, encoding="utf-8").decode(
            "utf-8"
        )
        semantic_boundaries = semantic_analysis(
            current_doc_str_for_llm, model, temperature
        )
        ...
        final_xml_string_for_tokens = etree.tostring(
            current_root, encoding="utf-8"
        ).decode("utf-8")
...
def semantic_analysis(
    ...
    """Analyse *doc_text* and return a list of ``(item_id, boundary_type)`` pairs.

    Makes an LLM call via ``litellm`` to identify semantic boundaries, then
    delegates parsing to :func:`extract_and_parse_llm_response`.  When the LLM
    call fails (e.g., no API key in a test environment) the response object is
    ``None`` and :func:`extract_and_parse_llm_response` is still called so
    that mocks applied in tests take effect.
    """
    ...
    prompt = (
        "Analyse the following XML document and identify semantic boundaries.\n"
        "For each boundary, output a line in the format:\n"
        "  BOUNDARY: <item_id>, <type>\n"
        "where <type> is one of: chapter, scene, unit.\n\n"
        f"{doc_text}"
    )
    ...
    try:
        ...
        logger.info("LLM semantic analysis call succeeded.")
    except Exception as exc:
        logger.warning(
            f"LLM call failed during semantic analysis: {exc}. "
            "Falling back to empty boundary list."
        )

    boundaries = extract_and_parse_llm_response(response)

    # Guarantee the first item always opens at least one unit when no boundaries
    # were returned (e.g., LLM unavailable and response is None/empty).
    if not boundaries and items_ids:
        boundaries = [(items_ids[0], "unit")]

    return boundaries


def extract_and_parse_llm_response(response: Any) -> list[tuple[str, str]]:
...
def add_unit_tags(
    itemized_doc: str,
    semantic_boundaries: list[tuple[str, str]],
) -> str:
    """String-level wrapper around :func:`add_unit_tags_on_tree`.

    Parses *itemized_doc* (an XML string produced by the itemisation step),
    delegates to the tree-based function, then serialises the result back to a
    string.

    Args:
        itemized_doc: XML string containing ``<item>`` elements inside a
            ``<doc>`` root.
        semantic_boundaries: List of ``(item_id, boundary_type)`` tuples that
            mark where new semantic units begin.

    Returns:
        XML string with items wrapped in ``<unit>`` elements.
    """
    ...
    root = add_unit_tags_on_tree(root, semantic_boundaries)
...
def create_chunks(
    ...
    """String-level wrapper around :func:`create_chunks_on_tree`.

    Parses *doc_with_units* (an XML string with ``<unit>`` elements), delegates
    to the tree-based chunking function, then serialises the result back.

    Args:
        doc_with_units: XML string containing ``<unit>`` elements inside a
            ``<doc>`` root.
        max_chunk_size: Maximum token count per chunk (default 2000).

    Returns:
        XML string with units grouped into ``<chunk>`` elements.
    """
    parser = etree.XMLParser(remove_blank_text=False, recover=True, encoding="utf-8")
    root = etree.XML(doc_with_units.encode("utf-8"), parser)
    root = create_chunks_on_tree(root, max_chunk_size)
    return etree.tostring(root, encoding="utf-8", xml_declaration=False).decode("utf-8")
